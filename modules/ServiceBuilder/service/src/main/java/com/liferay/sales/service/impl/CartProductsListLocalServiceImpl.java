...
public class CartProductsListLocalServiceImpl
    ...
    public List<SaleProduct> addProductToCartList(int quantity, long cartId, long stockId) {
        ...
        if (canAddProductInCart(cartId, stockId, quantity)) {
            try {
                ...
                for (int i = 0; i < quantity; i++) {
                    if (!existProduct(cartId, productListInput.get(i))) {
    ...
    private boolean canAddProductInCart(long cartId, long stockId, int quantity) {
    ...
    private boolean existProduct(long cartId, SaleProduct saleProduct) {
        boolean exist = false;
        for (SaleProduct product : getAllProductsByCartID(cartId)) {
